...
public class Run {
	...
	public static void main(String[] args) {
		...
		ArrayList<Double> crossProbabilitys = new ArrayList<Double>(), mutationProbabilitys = new ArrayList<Double>(),
				neighboringDistances = new ArrayList<Double>(), neighboringPenaltys = new ArrayList<Double>();
		...
		try {
		    ...
		    definition = Integer.parseInt(props.getProperty("definition"));
		    ...
		    for(String descVariables : props.getProperty("descVariables").split(" ")){
				...
				for(String descVariable : descVariables.split(";")){
					...
					//adds desc[0] variables of the same definition and limits
					for (int numCurrVar=0; numCurrVar<Integer.parseInt(desc[0]); numCurrVar++){
						variablesDescAux.add(new VariableDescription(definition,Double.parseDouble(desc[1]),Double.parseDouble(desc[2])));
					}
				}
		    ...
		    // Defines the  distance, or the distance for two individuals to be neighbors
			for(String nd : props.getProperty("nd").split(" ")){
				neighboringDistances.add(Double.parseDouble(nd));
			}

		    // Defines the fraction of the fitness of an individual that should be deduced for each neighbor of the individual
			for(String np : props.getProperty("np").split(" ")){
				neighboringPenaltys.add(Double.parseDouble(np));
			}

			String record = "idExecute;numInd;max_generations;acceptableDistance;maxFFcalls;neighboringDistance;neighboringPenality;" +
					"minCrossProbability;minMutationProbability;maxCrossProbability;maxMutationProbability;" +
					"fitnessFunction;selectFunction;crossFunction;mutationFunction;generation;countCallFF;" +
					"currentEuclideanDistance;mediaFitness;bestFitness;status;variables";
			...
			// Output printer
			printer = new PrinterThread(args[0]+"_outputEvaluation.csv");
			printer.start();
			printer.assign(record);

			Integer numExec = Integer.parseInt(props.getProperty("numExec"));
			
			long chronometerStart = (new Date()).getTime();
			
			for(int i = 0; i < fitnessFunctions.size(); i++){
				
				//System.out.println("Fitness functions computed: "+i+"/"+fitnessFunctions.size());
							...
							for(Integer numbersOfIndividual : numbersOfIndividuals){
								for(Double crossProbability : crossProbabilitys){
									for(Double mutationProbability : mutationProbabilitys){
										for(Double neighboringDistance : neighboringDistances){
											for(Double neighboringPenalty : neighboringPenaltys){
			...
			System.out.println("The processing of the tests ended, after "+((new Date()).getTime() - chronometerStart)+"ms."
					+ "\n The system will now print the results.");
			printer.shutdown();
