...
public class GAExecutor implements Runnable{
	...
	private Double acceptableDistance, neighboringDistance, neighboringPenalty;
	...
	public GAExecutor(Integer idExecute, PrinterThread printer, ArrayList<VariableDescription> variablesDesc,	Integer numInd, Integer max_generations, Double minCrossProbability, Double minMutationProbability,
		...
		this.neighboringPenalty = neighboringPenalty;
	...
	public void run(){
		...
		while(!stopCondition(countGeneration, countCallFF, idBestInd)){
			...
			Double computedFitnessValues[] = reFittedFitnessValues(fitnessValues, population);
			Integer listOfSelecteds[] = selectFunction.selectPopulation(computedFitnessValues);


				Population populationSelected = new Population(population.getNumInd(), population.getVariablesDesc());
				Double[] copyFitenssValues = fitnessValues.clone();

				for(int i = 0; i < population.getNumInd(); i++){
					populationSelected.setIndividual(i, population.getIndividual(listOfSelecteds[i]));
					//reference update
					fitnessValues[i] = copyFitenssValues[listOfSelecteds[i]];
				}

				population.setBinaryMatrix(populationSelected.getBinaryMatrix());
	...
	private Double[] getVariablesOfIndividual(Population population, int idIndividual) {
		...
		for(int j= 0; j < population.getVariablesDesc().size(); j++){
			variables[j] = population.getVaribleOfIndividual(idIndividual, j);
		}
		return variables;
	}
	...
	private Double[] reFittedFitnessValues(Double[] fitnessValues, Population population){
		if(neighboringPenalty!=0.0){//only executes if there is a neighboring penalty
			...
			for(int line=0; line<population.getNumInd(); line++){	//gets each individual
				...
				for(int collumn=0; collumn<population.getNumInd(); collumn++){	//to compare with each other individual
					if(line!=collumn){	//except the same
						if(line<collumn){ //calculates the euclidean distance if it has not been done before between those two individuals
							//calculates the distance and saves, saves also in the inverse position of the relations matrix
							distances[line][collumn] = distances[collumn][line] = euclideanDistanceCalculate(getVariablesOfIndividual(population, line), getVariablesOfIndividual(population, collumn));
						}
						
						//checks if the current distance saved in the distances matrix is the specified neighbouring distance
						if(distances[line][collumn] < neighboringDistance){	
				...
				Double penality = (numOfNeighbors*neighboringPenalty)*fitnessValues[line]*(fitnessValues[line] < 0 ? -1 : 1);

				/* decreases an amount of fitness value relative to the current penalty.
				 * the fitness value may get negative, but the program is able to handle that. */
				newFitnessValues[line] = fitnessValues[line] - penality;
			}
	...
	private void outputEvaluation(Population population, Double fitnessValues[], Integer idBestInd, Integer generation, boolean finish) {
		...
		record += ";"+maxFFcalls;
		record += ";"+neighboringDistance;
		record += ";"+neighboringPenalty;

		// Genetics probabilities
		record += ";"+minCrossProbability;
		record += ";"+minMutationProbability;
		
		record += ";"+maxCrossProbability;
		record += ";"+maxMutationProbability;
